...
describe('sn table: Rendering tests', () => {
  ...
  fs.readdirSync(paths.data).forEach((file) => {
    ...
    it(name, async () => {
      ...
      const element = await puppet.open(routes.render(fixture.key));

      // Capture screenshot
      const img = await puppet.screenshot(element);

      return expect(img).to.matchImageOf(name, { artifactsPath: paths.artifacts }, 0.01);
    });
  });
});
